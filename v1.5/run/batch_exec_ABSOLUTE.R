source("/xchip/cga_home/igleshch/CancerGenomeAnalysis/trunk/R/broad_utils/LSF_scatter.R")
source("/xchip/cga_home/igleshch/CancerGenomeAnalysis/trunk/R/broad_utils/qq_pval.R")
...
batch_exec_ABSOLUTE = function( ABSOLUTE_argv, obj.name, var_bsub_argv, queue="hour", overwrite=FALSE, dry_run=FALSE, num_solutions_plotted=3, wait=TRUE, summarize=TRUE )
{
...
apply_PP_review_and_liftover = function( pp.review.fn, obj.name, analyst.id )
{
  ...
  {
    called.segobj.list = run_PP_calls_liftover(pp.review.fn, analyst.id, modesegs.fn, out.dir.base, obj.name, verbose=TRUE )
...
extract_called_results = function( called.segobj.list, obj.name, analyst.id )
{
  out.dir.base = file.path( "ABSOLUTE_results", obj.name )
